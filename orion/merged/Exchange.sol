...
library LibUnitConverter {
    ...
    function decimalToBaseUnit(
        ...
        uint amount
    ) public view returns (int112 baseValue) {
        uint256 result;
    ...
    function baseUnitToDecimal(
        address assetAddress,
        uint amount
    ) public view returns (int112 decimalValue) {
        uint256 result;
...
interface PriceOracleInterface is PriceOracleDataTypes {
    ...
    ) external view returns (PriceDataOut[] memory);
}
...
library MarginalFunctionality {
    ...
    function getAssetPrice(
        address asset,
        address oracle
    ) internal view returns (uint64 price, uint64 timestamp) {
        PriceOracleInterface.PriceDataOut
            memory assetPriceData = PriceOracleInterface(oracle).assetPrices(
                asset
            );
    ...
    function calcAssets(
        ...
        internal
        view
        returns (bool outdated, int192 weightedPosition, int192 totalPosition)
    {
        ...
        for (uint256 i = 0; i < collateralAssetsLength; i++) {
            ...
            if (assetValue > 0) {
                ...
                outdated =
                    outdated ||
                    ((timestamp + priceOverdue) < block.timestamp);
            }
        }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < liabilitiesLength; i++) {
            ...
            (uint64 price, uint64 timestamp) = getAssetPrice(
                liability.asset,
                oracleAddress
            );
            // balance: i192, price u64 => balance*price fits i256
            ...
            overdue =
                overdue ||
                ((liability.timestamp + positionOverdue) < block.timestamp);
            outdated =
                outdated ||
                ((timestamp + priceOverdue) < block.timestamp);
        }
    ...
    function calcPosition(
        ...
        (
            bool _outdatedPrice,
            bool overdue,
            int192 _weightedPosition,
            int192 _totalPosition
        ) = calcLiabilities(
                liabilities,
                assetBalances,
                constants.user,
        ...
        uint64 lockedAmount = OrionVaultInterface(
            constants._orionVaultContractAddress
        ).getLockedStakeBalance(constants.user);
        int192 weightedStake = uint8Percent(
            int192(lockedAmount),
            constants.stakeRisk
        );
        ...
        bool incorrect = (liabilities[constants.user].length > 0) &&
            (lockedAmount == 0);
        if (_totalPosition < 0) {
    ...
    function partiallyLiquidate(
        ...
        require(
            initialPosition.state == PositionState.NEGATIVE ||
        ...
        PriceOracleInterface.PriceDataOut
            memory assetPriceData = PriceOracleInterface(
                constants._oracleAddress
            ).assetPrices(redeemedAsset);
        (uint64 price, uint64 timestamp) = (
            assetPriceData.price,
            assetPriceData.timestamp
        );
        ...
        require(
            int(finalPosition.state) < 3 && //POSITIVE,NEGATIVE or OVERDUE
                (finalPosition.weightedPosition >
                    initialPosition.weightedPosition),
            "E10"
        ); //Incorrect state position after liquidation
        if (finalPosition.state == PositionState.POSITIVE)
            require(
                finalPosition.weightedPosition < 10e8,
                "Can not liquidate to very positive state"
            );
    }
    ...
    function reimburseLiquidator(
        ...
        if (fromStake > 0) {
            OrionVaultInterface(orionVaultAddress).seizeFromStake(
                user,
                liquidator,
                uint64(fromStake)
            );
...
abstract contract OrionVault is ExchangeStorage, OwnableUpgradeSafe {
    ...
    function getStake(address user) public view returns (Stake memory stake) {
        ...
        if (
            ...
            (block.timestamp - stake.lastActionTimestamp) > releasingDuration
        ) {
    ...
    function getLockedStakeBalance(address user) public view returns (uint256) {
        ...
        if (
            stake.phase == StakePhase.LOCKED || stake.phase == StakePhase.FROZEN
        ) return stake.amount;
        return 0;
    }
...
contract Exchange is OrionVault, ReentrancyGuard {
    ...
    function updateOrderBalanceDebit(
        ...
        if (amountBase > 0) {
            ...
            tradeType =
                feeTradeType &
                _updateBalance(user, assetOut, amountOut);
            actualIn = amountIn;
            _updateBalance(
    ...
    function creditUserAssets(
        ...
        if (tradeType == 1 && remainingAmount > 0 && beforeBalance <= 0) {
            uint amountInBase = uint(
                LibUnitConverter.decimalToBaseUnit(asset, uint(remainingAmount))
            );
